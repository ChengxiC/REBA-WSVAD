...
import ucf_option as opt  # visual_length / test_list / model_path / classes_num
...
SIM_DROP_THR = 0.995
LIMIT_PER_CLASS = 300  # 每个类别的点数量

PCA_DIM = 64        # 稍大一些用于白化
PERPLEXITY = 30         # 20~30   # or 45–60，可减少小岛数量、让类团更连贯.
N_ITER = 5000           # 更久训练
EARLY_EXAG = 30.0    # 强化簇分离 ，  如果值较小 （12~16），可以减少早期的簇排斥现象
NO_PROGRESS = 1000
POST_JITTER = 0.05  # 极轻的抖动  0.005 ？
SEED = 5
OUT_PATH = "tsne_ucf.pdf"
...
def build_class_mapping(dataset, put_normal_first=True):
    uniq = list(sorted(set(map(str, dataset.df["label"].tolist()))))
    if put_normal_first and "Normal" in uniq:
        uniq.remove("Normal"); uniq = ["Normal"] + uniq
    label2idx = {c: i for i, c in enumerate(uniq)}
    return uniq, label2idx
...
def collect_clip_features_multiclass(
    ...
    for batch in loader:
        ...
        visual = visual.squeeze(0).to(device)  # [T, C]
        total_len = int(length)
        if total_len < maxlen:
            visual = visual.unsqueeze(0)       # [1, T, C]

        lengths = split_lengths(total_len, maxlen)                 # [B']
        padding_mask = get_batch_mask(lengths, maxlen).to(device)  # [B', maxlen]
        fused = model.encode_video(visual, padding_mask, lengths)  # [B', T, D]
        clips = take_valid_snippets(
            fused, lengths, stride=stride, per_video_max=per_video_max,
            sim_drop_thr=sim_drop_thr, rand_offset=rand_offset,
        )
        if clips.numel() == 0: continue
        Xs.append(clips.cpu().numpy())
...
def plot_tsne_layered(Z, y, class_names, out=OUT_PATH, show_centers=False):
    ...
    if normal_idx is not None:
        m0 = (y == normal_idx)
        if np.any(m0):
            plt.scatter(Z[m0, 0], Z[m0, 1], s=2, alpha=0.10, lw=0,
                        color="#b0b5ba", label=f"{class_names[normal_idx]}",
    ...
    for i, cls in enumerate(class_names):
        ...
        mi = (y == i)
        if np.sum(mi) == 0: continue
        plt.scatter(Z[mi, 0], Z[mi, 1], s=10, alpha=0.85, lw=0,
                    color=cmap(i % 20), label=f"{cls}",
    ...
    if show_centers:
        for i, cls in enumerate(class_names):
            ...
            plt.scatter(c[:,0], c[:,1], marker='*', s=250, linewidths=0.5,
                        edgecolors="white",
                        color="#1e88e5" if (normal_idx is not None and i==normal_idx) else "#333333",
                        zorder=3)
